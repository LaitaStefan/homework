...
namespace lab{
	...
	//incarca mai multe obiecte pentru a putea face interpolarea
	void loadMultipleObj(const std::string &filename1, const std::string &filename2, const std::string &filename3,
			unsigned int &vao, unsigned int& vbo, unsigned int &ibo, unsigned int &num_indices)	{
		
		unsigned int offset;

		//incarca vertecsii si indecsii din fisier
		std::vector<VertexFormat> vertices1, vertices2, vertices3;
		...
		offset = vertices1.size() * sizeof(VertexFormat);

		vertices1.insert(vertices1.end(), vertices2.begin(), vertices2.end());
		vertices1.insert(vertices1.end(), vertices3.begin(), vertices3.end());
		//indices1.insert(indices1.end(), indices2.begin(), indices2.end());
		//indices1.insert(indices1.end(), indices3.begin(), indices3.end());

		std::cout<<"Mesh Loader : am incarcat fisierele "<<filename1<<" "<<filename2<<" "<<filename3<<std::endl;
		...
		glVertexAttribPointer(3,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(offset));						//trimite pozitii pe pipe 3
		glEnableVertexAttribArray(4);
		glVertexAttribPointer(4,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(offset + sizeof(float)*3));	//trimite normale pe pipe 4
		glEnableVertexAttribArray(5);
		glVertexAttribPointer(5,2,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(offset + 2*sizeof(float)*3));	//trimite texcoords pe pipe 5

		glEnableVertexAttribArray(6);
		glVertexAttribPointer(6,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2*offset));					//trimite pozitii pe pipe 6
		glEnableVertexAttribArray(7);
		glVertexAttribPointer(7,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2*offset + sizeof(float)*3));	//trimite normale pe pipe 7
		glEnableVertexAttribArray(8);
		glVertexAttribPointer(8,2,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2*offset + 2*sizeof(float)*3));//trimite texcoords pe pipe 8

		vao = gl_vertex_array_object;
		vbo = gl_vertex_buffer_object;
		ibo = gl_index_buffer_object;
		num_indices = indices1.size();
	}
